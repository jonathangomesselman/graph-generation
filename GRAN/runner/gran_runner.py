...
class GranRunner(object):
  ...
  def __init__(self, config):
    ...
    ### load graphs
    self.graphs = create_graphs(config.dataset.name, data_dir=config.dataset.data_path, label=1)
  ...
  def test(self):
    ...
    if self.config.test.is_test_ER:
      ...
      def calc_nll(loader, model, save_path):
        iter_avg_nlls = []
        with torch.no_grad():
          for i in range(10):
            # nll, avg_nll = gran_data_nll(args, model, test_loader)
            avg_nlls = []
            test_iterator = loader.__iter__()
            for inner_iter in range(len(loader) // self.num_gpus):
              batch_data = []
              if self.use_gpu:
                for _ in self.gpus:
      ...
      anomaly_graphs = create_graphs(self.config.dataset.name, data_dir=self.config.dataset.data_path, label=0)
